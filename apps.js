...
const HUB_API_URL = CONFIG.API_URL || "";
...
const FALLBACK_APPS = [
  {
    ...
    icon: "P",
    color: "#8b6df6",
    url: "https://alicetletech.com/",
    secondaryLabel: "Admin",
    secondaryUrl: "https://alicetletech.com/admin",
    showAdmin: true
  },
  {
    id: "alicejens",
...
const APP_STYLE_BY_URL = {
  "https://alicetletech.com/": { id: "portfolio", categoryLabel: "Personal Brand", status: "live", color: "#8b6df6" },
  "https://alicetletech-afk.github.io/gyokoispace/": { id: "portfolio", categoryLabel: "Personal Brand", status: "live", color: "#8b6df6" },
  "https://alicetletech-afk.github.io/smallgirlsmallworld/": { id: "alicejens", categoryLabel: "AI Workspace", status: "demo", color: "#b46cff" },
  "https://gstarcadthai-spacee.github.io/gstarspace/": { id: "workspace", categoryLabel: "Internal Workspace", status: "internal", color: "#4f8cff" },
  "https://alicetletech-afk.github.io/PTCAD-Sales/": { id: "emailhub", categoryLabel: "Sales Tool", status: "internal", color: "#45b7d1" },
  "https://thaigstarcad.com/partner-centercampaign-manager": { id: "campaign", categoryLabel: "Marketing Operations", status: "internal", color: "#f0935b" },
  "https://thaigstarcad.com/": { id: "productcenter", categoryLabel: "Product Website", status: "live", color: "#2f7fe7" },
  "https://alicetletech-afk.github.io/kk-hatyai-car-rental/booking.html": { id: "kk", categoryLabel: "Family Business", status: "live", color: "#e0aa35" },
  "https://thaigstarcad.com/hub-access": { id: "hubaccess", categoryLabel: "Internal Gateway", status: "internal", color: "#68758f" }
};

const CATEGORY_STYLE = {
  personal: { categoryLabel: "Personal", status: "live", color: "#8b6df6" },
  work: { categoryLabel: "Work", status: "internal", color: "#4f8cff" },
  business: { categoryLabel: "Business", status: "live", color: "#e0aa35" },
  internal: { categoryLabel: "Internal", status: "internal", color: "#68758f" }
};
...
async function loadHubApps() {
  try {
    if (!HUB_API_URL) throw new Error("Hub API URL is not configured.");
    ...
    if (!response.ok) throw new Error("Hub API request failed.");
    ...
    APPS = FALLBACK_APPS.map((app, index) => ({ ...app, order: index + 1 }));
    return APPS;
  }
}
...
function normalizeApiApp(app, index) {
  ...
  const adminUrl = String(app.adminUrl || "").trim();
  const showAdmin = app.showAdmin === true && Boolean(adminUrl);
  ...
  return {
    ...
    url,
    secondaryLabel: showAdmin ? "Admin" : "",
    secondaryUrl: showAdmin ? adminUrl : "",
    showAdmin,
...
function slugify(value) {
  return String(value || "app").toLowerCase().trim()
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "") || "app";
}
